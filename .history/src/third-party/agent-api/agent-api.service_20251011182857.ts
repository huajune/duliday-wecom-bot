...
/**
 * Agent Computer User Open API 客户端服务
...
export class AgentApiService {
  ...
  constructor(
    ...
    this.httpClient.interceptors.response.use(
      (response) => {
        this.logger.log(
          `[Agent API] 收到响应: ${response.status} ${response.config.url}`,
        );
        return response;
      },
      (error) => {
        if (error.response) {
          this.logger.error(
            `[Agent API] 响应错误 ${error.response.status}:`,
            error.response.data,
          );
  ...
  async chat(params: {
    ...
    try {
      ...
      this.conversationManager.addMessage(conversationId, userMsg);
      ...
      // 发送请求
      const response = await this.httpClient.post<ChatResponse>('/chat', chatRequest);

      // 添加助手回复到历史
      if (response.data.messages && response.data.messages.length > 0) {
        const assistantMessages = response.data.messages.filter(m => m.role === 'assistant');
        this.conversationManager.addMessages(conversationId, assistantMessages);
      }
                ...
                switch (eventData.type) {
                  ...
                  case 'done':
                    ...
                    if (fullText) {
                      ...
                      this.conversationManager.addMessage(conversationId, assistantMsg);
                    }
                    if (onComplete) {
                      onComplete(fullText);
                    }
