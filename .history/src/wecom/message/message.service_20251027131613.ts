...
export class MessageService implements OnModuleDestroy {
  ...
  private async processMessageWithAI(messageData: EnterpriseMessageCallbackDto) {
    ...
    try {
      ...
      if (messageData.source !== MessageSource.NEW_CUSTOMER_ANSWER_SOP) {
        this.logger.debug(
      ...
      if (this.isDuplicateMessage(messageId)) {
        this.logger.debug(`[${scenarioType}] 跳过重复消息 [${messageId}]`);
        return;
      }
      ...
      if (this.isRecentlySentMessage(chatId, content)) {
        this.logger.debug(`[${scenarioType}] 跳过刚发送的消息（防止循环回复）[${messageId}]`);
        return;
      }

      // 群聊场景：检查是否被 @
      if (isRoom) {
        const isMentioned = this.checkMentioned(messageData, botWxid);
        if (!isMentioned) {
          this.logger.debug(`[群聊] 未被 @ ，跳过处理 [${messageId}]`);
          return;
        }
        this.logger.log(`[群聊] 检测到 @机器人，准备回复`);
      }
  ...
  private parseMessageData(messageData: EnterpriseMessageCallbackDto) {
    ...
    if (isTextPayload(messageData.messageType, messageData.payload)) {
      ...
      content = messageData.payload.pureText || messageData.payload.text;
    } else if (typeof messageData.payload === 'string') {
    ...
    // 根据 source 判断是否为群聊
    // 注意：企业级回调的 source 字段表示消息来源，不直接表示是否为群聊
    // 需要根据实际业务判断，这里暂时设为 false（私聊）
    const isRoom = false;

    return {
      token: messageData.token,
      messageId: messageData.messageId,
      contactId: messageData.imBotId, // 企业级回调没有 contactId，使用 imBotId
      contactName: '客户', // 企业级回调没有提供客户名称
      messageType: messageData.messageType,
      content,
      roomId: undefined, // 企业级回调没有 roomId
      roomName: undefined, // 企业级回调没有 roomName
      isRoom,
      chatId: messageData.chatId,
      botWxid: messageData.imBotId,
      botWeixin: undefined, // 企业级回调没有 botWeixin
      botId: messageData.botId,
      isSelf: messageData.isSelf,
      mentionSelf: false, // 企业级回调没有 mentionSelf 字段
