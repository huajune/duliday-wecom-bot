...
export class AgentRegistryService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    private readonly agentService: any, // 使用 any 避免循环依赖类型问题
  ) {
    // 读取配置
  ...
  private validateConfiguration(): void {
    ...
    if (this.availableModels.length > 0) {
      if (!this.availableModels.includes(this.configuredModel)) {
        this.logger.warn(
          `⚠️  配置的模型 "${this.configuredModel}" 不在可用列表中！` +
            `\n   可用模型: ${this.availableModels.join(', ')}`,
        );
      } else {
        this.logger.log(`✓ 配置的模型 "${this.configuredModel}" 验证通过`);
      }
    }
