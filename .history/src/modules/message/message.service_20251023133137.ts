...
export class MessageService implements OnModuleDestroy {
  ...
  private async processMessageWithAI(messageData: IncomingMessageData) {
    try {
      ...
      if (!validation.valid) {
        ...
        this.logger.error(`会话 ID: ${conversationId}，场景: ${scenario}`);
        // 配置验证失败，直接返回，不继续调用 Agent API
        return;
      }
  ...
  private sanitizeMessageData(messageData: IncomingMessageData): any {
    ...
    sanitized.contactId = this.sanitizeString(sanitized.contactId, 3, 3);
    sanitized.botWxid = this.sanitizeString(sanitized.botWxid, 3, 3);

    // 如果有 data 字段，也需要脱敏（新格式的数据都在这里）
    if (sanitized.data) {
      ...
      // 脱敏客户微信号等其他敏感字段
      if (sanitized.data.fromUser) {
        sanitized.data.fromUser = this.sanitizeString(sanitized.data.fromUser, 3, 3);
      }
    }
  ...
  private parseMessageData(messageData: IncomingMessageData) {
    ...
    if (messageData.data) {
      ...
      return {
        ...
        contactId: data.contactId || '', // 使用真实的 contactId，而不是 chatId
