...
import { AddFriendFromRoomDto } from './dto/add-friend-from-room.dto';
...
export class CustomerController {
  ...
  /**
   * 获取群列表
   * 访问: GET http://localhost:3000/customer/room/list
   */
  @Get('room/list')
  async getRoomList(
    @Query('token') token: string,
    ...
    return await this.customerService.getRoomList(token, current, pageSize, wxid);
  }
  ...
  /**
   * 群聊加好友
   * 访问: POST http://localhost:3000/customer/addFriend/room/send
   */
  @Post('addFriend/room/send')
  async addFriendFromRoom(@Body() body: AddFriendFromRoomDto) {
    return await this.customerService.addFriendFromRoom(body);
  }
}
