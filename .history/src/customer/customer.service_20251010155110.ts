...
export class CustomerService {
  ...
  async getCustomerListV2(
    ...
    try {
      const apiUrl = 'https://stride-bg.dpclouds.com/api/v2/customer/list';
  ...
  async getUserList(token: string, current?: number, pageSize?: number) {
    try {
      const apiUrl = 'https://stride-bg.dpclouds.com/stream-api/api/v1user/list';
      ...
      this.logger.log('获取小组成员列表成功');
      return result;
    } catch (error) {
      this.logger.error('获取小组成员列表失败:', error);
      throw error;
    }
  }
}
